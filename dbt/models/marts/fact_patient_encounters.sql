-- Fact: Patient encounters from HL7 ADT messages (grain: one row per
-- admission/visit_number)
-- Links to dim_patient via MRN, dim_provider via NPI
with
    encounters as (select * from {{ ref("int_patient_encounters") }}),

    dim_patients as (select * from {{ ref("dim_patient") }}),
    ...
    final as (
        ...
        left join
            dim_physicians dph on dprov.supervising_physician_key = dph.physician_key
    )
