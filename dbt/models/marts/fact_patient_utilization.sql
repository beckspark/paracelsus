...
with appointment_util as (
    select * from {{ ref('int_appointment_utilization') }}
),
...
daily_agg as (
    select
        ...
        count(distinct au.primary_diagnosis_code) filter (where au.has_encounter) as unique_diagnoses
...
final as (
    select
        ...
        case
            ...
            then round(da.completed_appointments::numeric / da.total_appointments * 100, 2)
            else 0
        end as completion_rate_pct,

        case
            when da.total_appointments > 0
            then round(da.no_show_appointments::numeric / da.total_appointments * 100, 2)
            else 0
        end as no_show_rate_pct,

        case
            when da.total_appointments > 0
            then round(da.cancelled_appointments::numeric / da.total_appointments * 100, 2)
            else 0
        ...
        case
            when da.total_appointments > 0
                and round(da.no_show_appointments::numeric / da.total_appointments * 100, 2) > 25
            then 'high_no_show'
            when da.total_appointments > 0
                and round(da.completed_appointments::numeric / da.total_appointments * 100, 2) >= 75
            then 'well_utilized'
            when da.total_appointments > 0
                and round(da.completed_appointments::numeric / da.total_appointments * 100, 2) >= 50
            then 'moderate'
            else 'underutilized'
