...
with
    cases as (select * from {{ ref("stg_oltp__cases") }}),

    reviews as (select * from {{ ref("stg_oltp__case_reviews") }}),
    ...
    case_with_reviews as (
        select
            ...
            -- Computed fields
            case
                when r.review_status = 'completed'
                then r.completed_at::date - r.due_date
            ...
            case
                when r.review_status = 'completed'
                then extract(day from (r.completed_at - c.created_at))
                else null
            ...
            case
                when r.review_status = 'overdue'
                then true
                when r.due_date < current_date and r.review_status = 'pending'
                then true
                else false
            end as is_overdue
