...
with
    admissions as (select * from {{ ref("stg_hl7__admissions") }}),

    discharges as (select * from {{ ref("stg_hl7__discharges") }}),

    emr_patients as (select * from {{ ref("stg_emr__patients") }}),

    emr_encounters as (select * from {{ ref("stg_emr__encounters") }}),
    ...
    adt_combined as (
        select
            ...
            -- Derived fields
            case
                when d.discharge_datetime is not null then 'discharged' else 'admitted'
            end as adt_status,
            ...
            case
                ...
                then
                    round(
                        extract(epoch from (d.discharge_datetime - a.admit_datetime))
                        / 3600.0,
                        1
                    )
    ...
    with_emr_patient as (
        select ac.*, ep.patient_id as emr_patient_id
