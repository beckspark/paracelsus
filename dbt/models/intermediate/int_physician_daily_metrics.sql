...
with
    case_reviews as (select * from {{ ref("int_case_review_status") }}),
    ...
    date_spine as (
        select
            generate_series(
                '{{ var("start_date") }}'::date, current_date, '1 day'::interval
            )::date as date_key
    ),

    -- Cross join physicians with dates to ensure we have a row for each combination
    physician_dates as (
        select p.physician_id, p.total_providers_supervised, d.date_key
    ...
    daily_review_metrics as (
        select
            ...
            count(
                case
                    when review_status = 'pending' and due_date >= review_date then 1
                end
            ) as cases_pending_review,
            ...
            avg(
                case when review_status = 'completed' then days_to_complete_review end
